...
(function(Scratch) {
  ...
  // Load the Hjson library
  /*
  const hjsonScript = document.createElement('script');
  hjsonScript.src = 'https://unpkg.com/hjson@3.2.2/bundle/hjson.min.js';
  document.head.appendChild(hjsonScript);
  const waitForHjson = () => {
    return new Promise((resolve) => {
      if (window.Hjson) {
        return resolve();
      }
      hjsonScript.onload = resolve;
    });
  };
  */


  class HjsonExtension {
    getInfo() {
      return {
        id: 'hjson',
        name: 'Hjson',
        ...
        blocks: [
          ...
          },
          {
            ...
            arguments: {
              OBJECT: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: '{"key":"value","array":[1,2,3]}'
              }
            }
          },
          {
            opcode: 'stringifyWithOptions',
            blockType: Scratch.BlockType.REPORTER,
            text: 'stringify [OBJECT] with spaces [SPACES]',
            arguments: {
              OBJECT: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: '{"key":"value","array":[1,2,3]}'
          ...
          {
            opcode: 'getValueFromKey',
            blockType: Scratch.BlockType.REPORTER,
            text: 'get [KEY] from Hjson [OBJECT]',
            arguments: {
              KEY: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'key'
    ...
    async parse(args) {
      try {
        const parsed = Hjson.parse(args.TEXT);
      ...
      try {
        ...
        if (typeof object === 'string') {
          try {
            ...
            // If it's not valid JSON, try parsing as Hjson
            try {
              object = Hjson.parse(object);
            } catch (e2) {
              return 'Error: Invalid input object';
            }
          }
        }
        return Hjson.stringify(object);
      } catch (e) {
        return 'Error: ' + e.message;
      }
    }

    async stringifyWithOptions(args) {
      
      try {
        let object = args.OBJECT;
        const spaces = Number(args.SPACES) || 0;
    ...
    async getValueFromKey(args) {
      
      try {
        let object = args.OBJECT;
        const key = args.KEY;
        
        // If the input is a string, parse it as Hjson
        if (typeof object === 'string') {
          object = Hjson.parse(object);
        }
    ...
    async isValidHjson(args) {
      
      try {
        Hjson.parse(args.TEXT);
        return true;
      } catch (e) {
        return false;
      }
  ...
  Scratch.extensions.register(new HjsonExtension());
})(Scratch);